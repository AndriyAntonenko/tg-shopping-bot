...
class ProductService:
    ...
    async def get_products_list(self, params: GetProductsListParams):
        ...
        condition = (
            """p.id <= ? AND (o.id IS NULL OR o.status = ?)"""
            if params.cursor > 0
            else "(o.id IS NULL OR o.status = ?)"
        )
        query = f"""
    ...
    """
        args = (
            (params.cursor, OrderStatus.CANCELED.value, params.limit + 1)
            if params.cursor > 0
            else (
        ...
        products = map(
            from_db_row_to_product_item, products_raws[: params.limit]
        )  # Return only the requested limit
        fetch_next_cursor = None
        if len(products_raws) > params.limit:
            fetch_next_cursor = products_raws[params.limit][
                0
    ...
    async def add_product(
        self, name: str, description: str, image_url: str, price: float, currency: str
    ):
        conn = await get_db_connection()
        cursor_obj = await conn.cursor()
        query = """
