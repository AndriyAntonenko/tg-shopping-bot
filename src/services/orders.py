...
class OrdersService:
  ...
  async def get_pending_orders_list(self, ids: list[int] | None) -> list[OrderItemDetailed]:
    conn = await get_db_connection()
    cursor = await conn.cursor()

    ids_condition = ' AND o.id IN ({})'.format(','.join('?' * len(ids))) if ids else ''
    await cursor.execute(
      '''
      SELECT
        o.id AS id,
  ...
  async def update_order_status(self, order_id: int, new_status: OrderStatus) -> bool:
    ...
    await cursor.execute(
      ...
      WHERE id = ? AND status = ?;
      ''',
      (new_status.value, order_id, OrderStatus.PENDING.value) # Only pending orders can be updated
    )
